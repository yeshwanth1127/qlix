"""Single point of contact between Python and the Rust ``luna_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``luna_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import luna_rust  # type: ignore[import-untyped]
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from qlix.luna.security.injection_scanner import (
        InjectionScanResult,
    )
    from qlix.luna.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from qlix.luna.tools.storage._stubs import RetrievalResult
