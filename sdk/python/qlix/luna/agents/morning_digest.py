...
from qlix.luna.agents.digest_store import DigestArtifact, DigestStore
from qlix.luna.core.registry import AgentRegistry
from qlix.luna.core.types import Message, Role, ToolCall


def _load_persona(persona_name: str) -> str:
    """Load a persona prompt file by name."""
    search_paths = [
        Path("configs/luna/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".luna" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._persona = kwargs.pop("persona", "luna")
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from qlix.luna.agents.digest_evaluator import DigestEvaluator
