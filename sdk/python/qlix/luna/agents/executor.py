...
from qlix.luna.agents._stubs import AgentResult
from qlix.luna.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a LunaSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after LunaSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from qlix.luna.agents import AgentRegistry
        ...
        # Resolve engine + model from LunaSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in LunaSystem")
        ...
        if router_policy_key and self._system:
            try:
                from qlix.luna.core.registry import RouterPolicyRegistry
                from qlix.luna.learning.routing.types import (
        ...
        if tool_names:
            try:
                from qlix.luna.server.agent_manager_routes import (
    ...
    def _save_trace(
        ...
        from qlix.luna.core.types import StepType, Trace, TraceStep
