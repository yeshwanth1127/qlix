"""Load system prompt and few-shot overrides from $LUNA_HOME.
...
- System prompts: ``$LUNA_HOME/agents/{name}/system_prompt.md``
- Few-shot exemplars: ``$LUNA_HOME/agents/{name}/few_shot.json``
...
def _luna_home() -> Path:
    """Resolve $LUNA_HOME, defaulting to ~/.luna."""
    return Path(os.environ.get("LUNA_HOME", "~/.luna")).expanduser()


def load_system_prompt_override(agent_name: str) -> str | None:
    """Return the override prompt for *agent_name*, or ``None``.

    Looks for ``$LUNA_HOME/agents/<agent_name>/system_prompt.md``.
    ``LUNA_HOME`` defaults to ``~/.luna`` when unset.
    """
    home = _luna_home()
...
def load_few_shot_exemplars(
    ...
    Looks for ``$LUNA_HOME/agents/<agent_name>/few_shot.json``.
