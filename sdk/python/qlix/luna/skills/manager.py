...
from qlix.luna.skills.dependency import validate_dependencies
from qlix.luna.skills.executor import SkillExecutor, SkillResult
from qlix.luna.skills.loader import discover_skills
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.luna/learning/skills/ if config can't be loaded.
            try:
                from qlix.luna.core.config import load_config
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.luna/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.luna.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from qlix.luna.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("luna", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["luna"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.luna.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("luna", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.luna/skills/discovered/``).  Returns a list of dicts with
        ...
        """
        try:
            from qlix.luna.learning.agents.skill_discovery import SkillDiscovery
        except ImportError as exc:
            raise RuntimeError(
                "Skill discovery requires the learning subsystem, which is "
                "not bundled with the Qlix SDK build. Run skill discovery "
                "via the Qlix backend instead."
            ) from exc
        ...
        if output_dir is None:
            output_dir = Path("~/.luna/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "luna (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.luna/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from qlix.luna.skills.loader import load_skill_directory
