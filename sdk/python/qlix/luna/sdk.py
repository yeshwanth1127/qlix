"""High-level Python SDK for Luna."""
...
from qlix.luna.system import LunaSystem, SystemBuilder
from qlix.luna.telemetry.instrumented_engine import InstrumentedEngine
...
class MemoryHandle:
    ...
    def _get_backend(self) -> Any:
        ...
        if not MemoryRegistry.contains(key):
            ...
            try:
                from qlix.luna.tools.storage.sqlite import SQLiteMemory  # noqa: F401
            except ImportError:
                pass
    ...
    def index(
        ...
        *,
        ...
        from qlix.luna.tools.storage.chunking import ChunkConfig
        from qlix.luna.tools.storage.ingest import ingest_path
...
class Luna:
    ...
    def version(self) -> str:
        """Return the Luna version string."""
        return luna.__version__
    ...
    def _ensure_engine(self) -> None:
        ...
        try:
            import qlix.luna.engine  # noqa: F401
        except ImportError:
            pass
        ...
        if self._config.telemetry.gpu_metrics:
            try:
                from qlix.luna.telemetry.energy_monitor import create_energy_monitor
    ...
    def _run_agent(
        ...
        *,
        ...
        import qlix.luna.agents  # noqa: F401
        from qlix.luna.agents._stubs import AgentContext
        ...
        if tools:
            import qlix.luna.tools  # noqa: F401
            from qlix.luna._runtime_helpers import _build_tools
        ...
        if agent_name == "morning_digest" and hasattr(self._config, "digest"):
            ...
            # TextToSpeechTool was removed in the SDK build (speech backend
            # is server-side only), so the digest agent here runs text-only.
            from qlix.luna.tools.digest_collect import DigestCollectTool

            digest_tools = [DigestCollectTool()]
        ...
        if context and self._config.agent.context_from_memory:
            try:
                from qlix.luna._runtime_helpers import _get_memory_backend
                from qlix.luna.tools.storage.context import (
                    ContextConfig,
                    inject_context,
                )
    ...
    def list_engines(self) -> List[str]:
        ...
        from qlix.luna.core.registry import EngineRegistry
...
__all__ = ["Luna", "LunaSystem", "MemoryHandle", "SystemBuilder"]
