"""MCP Server — wraps Luna tools as MCP-discoverable tools."""
...
from qlix.luna.core.types import ToolCall
from qlix.luna.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes Luna tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        # Built-in API tools
        try:
            from qlix.luna.tools.calculator import CalculatorTool

            _tool_classes.append(CalculatorTool)
        except ImportError:
            pass
        try:
            from qlix.luna.tools.think import ThinkTool

            _tool_classes.append(ThinkTool)
        except ImportError:
            pass
        try:
            from qlix.luna.tools.file_read import FileReadTool

            _tool_classes.append(FileReadTool)
        except ImportError:
            pass
        try:
            from qlix.luna.tools.web_search import WebSearchTool

            _tool_classes.append(WebSearchTool)
        except ImportError:
            pass
        try:
            from qlix.luna.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from qlix.luna.tools.repl import ReplTool
        ...
        # Storage MCP tools
        try:
            from qlix.luna.tools.storage_tools import (
                MemoryIndexTool,
                MemoryRetrieveTool,
                MemorySearchTool,
                MemoryStoreTool,
        ...
        # Channel MCP tools
        try:
            from qlix.luna.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )
        ...
        try:
            from qlix.luna.tools.llm_tool import LLMTool
        ...
        try:
            from qlix.luna.tools.retrieval import RetrievalTool
