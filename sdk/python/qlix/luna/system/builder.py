"""Config-driven fluent builder that wires up a LunaSystem."""
...
from qlix.luna.core.config import LunaConfig, load_config
from qlix.luna.core.events import EventBus, get_event_bus
...
class SystemBuilder:
    """Config-driven fluent builder for LunaSystem."""
    ...
    def build(self) -> LunaSystem:
        """Construct a fully wired LunaSystem.

        Raises ``IdentityGateError`` if no active QlixSDK is registered.
        """
        from qlix.luna._gate import require_active_sdk
        ...
        from qlix.luna.security import setup_security

        sec = setup_security(config, engine, bus)
        engine = sec.engine

        if telemetry_enabled:
            from qlix.luna.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.luna/agents.db").expanduser()
                )
        ...
        if speech_enabled:
            try:
                from qlix.luna.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: LunaConfig):
        from qlix.luna.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: LunaConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_memory(self, config):
        try:
            import qlix.luna.tools.storage  # noqa: F401 -- trigger registration
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import qlix.luna.channels  # noqa: F401 -- trigger registration
            from qlix.luna.core.registry import ChannelRegistry
            from qlix.luna.system._channel_kwargs import build_channel_kwargs
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: LunaConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from qlix.luna.core.config import DEFAULT_CONFIG_DIR
            from qlix.luna.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from qlix.luna.learning.training.lora import LoRATrainingConfig
            from qlix.luna.traces.store import TraceStore
    ...
    def _discover_external_mcp(self, server_cfg) -> List[BaseTool]:
        ...
        from qlix.luna.mcp.transport import StdioTransport, StreamableHTTPTransport
        from qlix.luna.tools.mcp_adapter import MCPToolProvider
