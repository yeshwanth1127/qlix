"""Browser automation tools — Playwright or agent-browser via Luna driver."""
...
logger = logging.getLogger("qlix.luna.tools.browser")
...
def _browser_unavailable_message(exc: Exception) -> str:
    msg = str(exc)
    if "playwright" in msg.lower():
        return "playwright not installed. Install with: pip install -e \".[browser]\""
    if "agent-browser" in msg.lower():
        return (
            "agent-browser binary not available. "
            "Set LUNA_BROWSER_ENGINE=playwright for local dev, or npm install -g agent-browser."
        )
    return msg
...
class BrowserNavigateTool(BaseTool):
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        try:
            meta = _driver().navigate(url, wait_for=wait_for)
            ...
            return ToolResult(
                tool_name="browser_navigate",
                content=_browser_unavailable_message(exc),
                success=False,
            )
...
class BrowserClickTool(BaseTool):
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        try:
            _driver().click(selector, by_text=by_text)
            return ToolResult(
                tool_name="browser_click",
...
class BrowserTypeTool(BaseTool):
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        try:
            _driver().fill(selector, text, clear=clear)
            return ToolResult(
                tool_name="browser_type",
...
class BrowserScreenshotTool(BaseTool):
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        try:
            screenshot_bytes = _driver().screenshot_bytes(full_page=full_page)
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        try:
            ...
            if extract_type == "text":
                content = drv.extract_text(selector)
            ...
            if extract_type == "links":
                links = drv.extract_links(selector)
                lines = []
                for link in links:
                    text = link.get("text", "")
                    href = link.get("href", "")
            ...
            tables_text = drv.extract_tables(selector)
...
# Legacy alias for tests patching _session.page
class _SessionShim:
    @property
    def page(self):
        return get_browser_driver().page
