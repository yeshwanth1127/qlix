...
class BaseTool(ABC):
    ...
    def to_openai_function(self) -> Dict[str, Any]:
        ...
        from qlix.luna.tools.description_loader import (
...
class ToolExecutor:
    ...
    def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        Refuses to run when no QlixSDK identity is active. The audit-billing
        wrapper (``QlixToolExecutor``) installs the SDK before delegating
        here; bypassing the wrapper hits this gate.
        """
        ...
        if taint_set is not None:
            try:
                from qlix.luna.security.taint import TaintSet, check_taint
        ...
        if result.success:
            try:
                from qlix.luna.security.taint import auto_detect_taint
