...
User configuration lives at ``~/.luna/config.toml``.  ``load_config()``
...
DEFAULT_CONFIG_DIR = Path.home() / ".luna"
DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "config.toml"


def _ensure_config_dir() -> Path:
    """Ensure the config directory exists with restrictive permissions."""
    from qlix.luna.security.file_utils import secure_mkdir
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from qlix.luna.intelligence.model_catalog import BUILTIN_MODELS
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.luna/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through Luna. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.luna/whatsapp_auth
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.luna/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.luna/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.luna/SOUL.md"
    memory_path: str = "~/.luna/MEMORY.md"
    user_path: str = "~/.luna/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.luna/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/luna/skill-index.git"
    index_dir: str = "~/.luna/skill-index/"
    max_depth: int = 5
...
@dataclass
class LunaConfig:
    """Top-level configuration for Luna."""
...
# Sections that users may set via ``luna config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(LunaConfig.__dataclass_fields__.keys()) - {"hardware"}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``LunaConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import qlix.luna.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "LunaConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> LunaConfig:
    """Detect hardware, build defaults, overlay TOML overrides.

    Parameters
    ----------
    path:
        Explicit config file. If not set, uses ``LUNA_CONFIG`` when set,
        otherwise ``~/.luna/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = LunaConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("LUNA_CONFIG"):
        config_path = Path(os.environ["LUNA_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``luna init``)
# ---------------------------------------------------------------------------
...
# Full reference config: luna init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.luna/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.luna/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.luna/whatsapp_auth
# assistant_name = "Luna"
