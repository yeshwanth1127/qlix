...
class OAuthProvider:
    ...
    # Filenames in ~/.luna/connectors/ to save tokens to
...
def get_client_credentials(
    ...
    Checks credential files in ``~/.luna/connectors/`` and falls
    back to environment variables ``LUNA_{NAME}_CLIENT_ID`` and
    ``LUNA_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"LUNA_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Luna.</p>"
                    b"</body></html>"
                )
