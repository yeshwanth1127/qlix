...
router = APIRouter()
logger = logging.getLogger("openjarvis.server.routes")
...
def _agent_tool_results_payload(result) -> list[dict[str, Any]]:
    """Serialize agent tool results for the OpenAI-compatible HTTP response."""
    out: list[dict[str, Any]] = []
    for tr in getattr(result, "tool_results", None) or []:
        raw_md = getattr(tr, "metadata", None) or {}
        slim_md: dict[str, Any] = {
            k: raw_md[k]
            for k in ("url", "opened_with", "title", "status")
            if k in raw_md
        }
        out.append(
            {
                ...
                "content": _safe_preview(getattr(tr, "content", "") or "", 2000),
                "metadata": slim_md,
            }
        )
    return out
...
async def chat_completions(request_body: ChatCompletionRequest, request: Request):
    ...
    try:
        # Best-effort request metadata for debugging backend->OpenJarvis traffic.
        client = getattr(request, "client", None)
        client_host = getattr(client, "host", None) if client else None
        msg_count = len(request_body.messages or [])
        last_user = ""
        if request_body.messages:
            for m in reversed(request_body.messages):
                if getattr(m, "role", "") == "user" and getattr(m, "content", ""):
                    ...
                    break
        logger.info(
            "chat_completions_in model=%s stream=%s tools=%s agent_present=%s client=%s msgs=%d last_user=%r",
            model,
            bool(request_body.stream),
            len(request_body.tools or []),
            agent is not None,
            client_host,
            msg_count,
            _safe_preview(last_user),
        )
    except Exception:
        logger.debug("chat_completions_in log failed", exc_info=True)
    ...
    if request_body.stream:
        ...
        if agent is not None and bus is not None and request_body.tools:
            resp = await _handle_agent_stream(agent, bus, model, request_body)
        else:
            resp = await _handle_stream(engine, model, request_body, complexity_info)
        logger.info(
            "chat_completions_out stream=true elapsed_ms=%d",
    ...
    if agent is not None:
        ...
        logger.info(
            "chat_completions_out mode=agent elapsed_ms=%d out_chars=%d",
            int((time.perf_counter() - started) * 1000),
            len((resp.choices[0].message.content or "") if resp.choices else ""),
        )
        return resp
    ...
    logger.info(
        "chat_completions_out mode=direct elapsed_ms=%d out_chars=%d",
        int((time.perf_counter() - started) * 1000),
        len((resp.choices[0].message.content or "") if resp.choices else ""),
    )
    return resp
...
def _handle_agent(
    ...
    try:
        ...
        try:
            ...
            _logp = None
            while _p.parent != _p:
                if (_p / "IRA" / "ira" / "OpenJarvis").is_dir():
                    _logp = _p / "debug-c0c673.log"
                    break
                _p = _p.parent
            if _logp is None:
                _logp = Path.cwd() / "debug-c0c673.log"
            _log_targets = [_logp, _logp.parent / ".cursor" / "debug-c0c673.log"]
            _trs = getattr(result, "tool_results", None) or []
            _summary = [
                {
                    "tool_name": getattr(tr, "tool_name", type(tr).__name__),
                    "success": getattr(tr, "success", None),
                    "content_preview": _safe_preview(getattr(tr, "content", "") or "", 200),
                }
                for tr in _trs
            ]
            _line = (
                json.dumps(
                    {
                        ...
                        "location": "routes.py:_handle_agent",
                        "message": "agent.run completed",
                        "data": {
                            "request_has_tools_key": getattr(req, "tools", None) is not None,
                            "request_tools_len": len(req.tools) if getattr(req, "tools", None) else 0,
                            "model": model,
                            "turns": getattr(result, "turns", None),
                            "tool_results_count": len(_summary),
                            "tool_results": _summary,
                            "reply_preview": _safe_preview(result.content or "", 300),
                        },
            ...
            for _lt in _log_targets:
                try:
                    _lt.parent.mkdir(parents=True, exist_ok=True)
                    with open(_lt, "a", encoding="utf-8") as _f:
                        _f.write(_line)
                except Exception:
                    pass
        except Exception:
    ...
    _oj_tools = _agent_tool_results_payload(result)
    return ChatCompletionResponse(
        model=model,
        choices=[
            Choice(
        ...
        usage=usage,
        complexity=complexity_info,
        openjarvis_agent_tool_results=_oj_tools or None,
    )
