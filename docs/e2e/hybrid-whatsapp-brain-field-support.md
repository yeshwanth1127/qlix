...
End-to-end use case: an officer sends a WhatsApp message; the **hybrid** agent reads company playbook from **AI Brain**, opens a log file on the **local PC**, and replies on WhatsApp when done.
...
2. Set **default agent** to `local` (or always use `@local:` prefix).

## 4. WhatsApp test message

Send to your linked business number:

```
@local #brain: Follow our field support playbook. On my PC read C:\Users\admin\source\repos\autocad-final\autocad-final\bin\Debug\AgentDebug.log, open it on my screen in Notepad, list the last 5 error lines, then summarize for me here.
```
...
| Step | What you see |
|------|----------------|
| Immediate reply | `Queued — local is on it. (brain on, local PC)` |
...
| Dashboard | **Active runs** while queued/running; then agent chat activity (s3_read_file, s3_python, s3_open_file, …) |
